...
\begin{enumerate}
  ...
  \item Biểu diễn văn bản và điều khoản bằng LegalDocument và LegalProvision, kèm định danh ổn định và khoảng hiệu lực \code{[effective\_from, effective\_to)}.
...
\land
\left(p.\operatorname{effective\_to}=\varnothing\ \lor\ d<p.\operatorname{effective\_to}\right)
\land
\left(p.\operatorname{review\_status}=\text{ACCEPTED}\right).
\end{equation}
...
\begin{center}
\code{\{loai-van-ban\}-\{so\}-\{nam\}\_\_dieu-\{n\}\_\_khoan-\{n\}\_\_diem-\{chu-cai\}}
\end{center}
...
\begin{enumerate}
  \item exact legal lookup theo payload filter và SQL cho định danh như \code{168/2024/NĐ-CP}, \code{Điều 7}, \code{Khoản 4}, \code{Điểm a};
...
GET/POST & \apipath{/api/v1/reviews} & Danh sách và quyết định review\\
