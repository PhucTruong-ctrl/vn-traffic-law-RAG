...
Record the date, browser, result, and evidence path beside every item. Leave each item unchecked until it has been observed in a real browser. The active local surface is Next.js at `http://127.0.0.1:3000` with FastAPI at `http://127.0.0.1:8000/api/v1`; frontend API routing uses `NEXT_PUBLIC_API_URL` (normally `http://127.0.0.1:8000`). Authentication uses Supabase Auth, persistence uses Supabase REST, retrieval uses Qdrant (`traffic_law`), and answers pass the deterministic evidence gate before reaching the single OpenRouter generator. Superseded RAGFlow/worker/Redis/MinIO/PostgreSQL topologies are not release prerequisites.
