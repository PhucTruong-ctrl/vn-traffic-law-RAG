...
The born-digital parser fixtures support a parser-native metrics benchmark. **Raw numbers only; make NO superiority conclusion between parsers** (see §8 for the historical M1 assessment). The gitignored `data/evaluation/` tree is the source of truth for raw artifacts (immutable per run_id; corrections are new runs, never rewrites).

In its originating environment, this report was generated by `python -m app.evaluation.suites.suite_a report --runs data/evaluation/suite-a-first-pass --out docs/evaluation/suite-a-first-pass-report.md`. That command remains historical provenance, not an active release command.
...
The historical runs recorded Tesseract `5.5.3`, Vietnamese language data, `psm=3`, CPU-only processing, and a 300 DPI born-digital policy. A separate historical OCR decision artifact, `run-20260809-120116-24f592`, used pages 2 and 7 of the scan-only NĐ 168 PDF.
...
The historical P3 run recommended Docling for searchable PDFs, Docling OCR followed by MinerU fallback for scans, parser comparison for complex tables, and review for ambiguous or low-provenance OCR. The born-digital fixtures did not exercise all of those routes. The historical report claimed M1 on its then-defined parser-foundation gate; do not read that claim as current MVP release readiness.
