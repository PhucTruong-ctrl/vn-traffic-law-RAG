...
`backend/app/rag/retrieval.py` cấu hình Qdrant local hoặc remote, dense embeddings OpenAI-compatible/OpenRouter, FastEmbed sparse BM25, tìm kiếm kết hợp (hybrid), exact metadata filtering theo reference, temporal filtering theo `effective_from/effective_to`, sibling completion và cross-reference expansion có giới hạn. Khi provider embedding lỗi hoặc quá hạn, store vẫn được dựng (bỏ qua bước kiểm tra collection vốn phải gọi embedding) và truy vấn rơi về chỉ mục sparse BM25 cục bộ; log chỉ ghi một dòng cảnh báo thay vì traceback.
