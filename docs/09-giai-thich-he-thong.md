...
`backend/app/rag/service.py` dùng bộ phân tích yêu cầu LLM-first strict JSON (phương án dự phòng tất định), truy xuất đa truy vấn cho các `expanded_queries`, hợp nhất điểm kiểu RRF, enrichment sibling/chế tài/cross-reference, giới hạn diversity, relevance filter graceful phương án dự phòng, rồi gọi bộ sinh câu trả lời. Generator hỗ trợ trả lời một phần và chỉ từ chối toàn bộ bằng `CANONICAL_REFUSAL` khi không có thông tin liên quan. `backend/app/rag/verification.py` chạy `sanitize_response`: parse citation từ chính câu trả lời, đối chiếu metadata đã tìm kiếm căn cứ, loại citation/claim không khớp nhưng giữ phần trả lời còn lại. `backend/app/rag/evidence.py` hiện chỉ còn `ABSTENTION_MESSAGE`.
