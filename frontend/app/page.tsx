      ...
      <div className="chat-shell">
        ...
        <section className="chat-workspace" aria-label="Khu vực tra cứu">
          ...
          <div className="conversation" aria-busy={loading}>
            ...
            <form onSubmit={submit} className="query-form"><label htmlFor="question">Câu hỏi</label><div className="query-input-row"><textarea id="question" required value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Ví dụ: Mức phạt khi vượt đèn đỏ là bao nhiêu?" rows={2} /><button type="submit" disabled={Boolean(loading || !question.trim())}>{loading ? "Đang tra cứu..." : "Gửi câu hỏi"}<span aria-hidden="true">↑</span></button></div><span className="form-hint">Nhấn Gửi câu hỏi để bắt đầu tra cứu</span></form>
