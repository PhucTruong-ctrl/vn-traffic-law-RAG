    ...
    try {
      ...
      setError(
        cause instanceof TypeError
          ? "Không thể kết nối dịch vụ mục đã lưu. Kiểm tra mạng rồi thử lại."
          : cause instanceof Error
            ? cause.message
            : "Không thể tải danh sách đã lưu. Thử lại sau vài giây.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function remove(item: SavedItem) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError("Không thể xóa mục đã lưu. Thử lại sau vài giây.");
      }
    } catch {
      setError("Không thể kết nối dịch vụ mục đã lưu. Kiểm tra mạng rồi thử lại.");
    } finally {
      setDeleting(false);
    }
  }
