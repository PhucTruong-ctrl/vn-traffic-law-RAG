...
test.describe("Sprint 5 answer flow", () => {
  ...
  test("renders the chat-only home form and validates a blank question", async ({
    page,
  }) => {
    await page.goto("/");
    await expect(
      page.getByRole("heading", { name: "Hỏi đáp pháp luật giao thông" }),
    ).toBeVisible();
  ...
  test("submits only the chat question and renders a verified response", async ({
    ...
    await expect(
      page.getByText("Mức phạt được xác định theo quy định hiện hành."),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Nghị định 100/2019/NĐ-CP" }),
    ).toBeVisible();
  });
  ...
  test("renders an abstention from the mock API boundary", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Gửi", exact: true }).click();
    await expect(
      page.getByRole("heading", { name: "Chưa đủ căn cứ để kết luận" }),
    ).toBeVisible();
  ...
  test("shows the submitted question in the chat loading bubble", async ({
    page,
  }) => {
...
test.describe("Delayed response accessibility", () => {
  test("shows neutral in-flight status while a chat response is pending", async ({
    page,
  }) => {
    let releaseResponse!: () => void;
    const responseReady = new Promise<void>((resolve) => {
