...
def temporal_validity_accuracy(citations: Sequence[Mapping[str, object]], query_date: date) -> float | None:
    if not citations:
        return None
    return sum(is_temporally_valid(item, query_date) for item in citations) / len(citations)


def temporal_leakage_rate(citations: Sequence[Mapping[str, object]], query_date: date) -> float | None:
...
def _aggregate(values: Mapping[str, float | None], categories: Mapping[str, str]) -> MetricReport:
    ...
    if not eligible:
        return MetricReport.na("no citations eligible for temporal evaluation", per_query=dict(values))
    grouped: dict[str, list[float]] = defaultdict(list)
    for key, value in eligible.items():
        grouped[categories.get(key, "uncategorized")].append(value)
    return MetricReport(sum(eligible.values()) / len(eligible), len(eligible), len(eligible), per_query=dict(values), by_category={key: sum(v) / len(v) for key, v in grouped.items()})
...
def evaluate_temporal(
    ...
    values = {"temporal_validity_accuracy": {}, "temporal_leakage_rate": {}}
    ...
    for index, record in enumerate(records):
        ...
        citations = record.get("citations", record.get("results", []))
        if query_date is None:
            valid = leakage = None
        else:
            valid = temporal_validity_accuracy(citations, query_date)  # type: ignore[arg-type]
            leakage = temporal_leakage_rate(citations, query_date)  # type: ignore[arg-type]
...
__all__ = ["is_temporally_valid", "temporal_validity_accuracy", "temporal_leakage_rate", "evaluate_temporal"]
