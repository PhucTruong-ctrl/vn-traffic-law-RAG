...
def all_required_evidence_at_10(
    required: Iterable[object], retrieved_evidence: Sequence[object]
) -> bool | None:
...
def cross_reference_resolution_recall(
    expected_targets: Iterable[str], resolved_targets: Iterable[str]
) -> float | None:
...
def multi_hop_evidence_completeness(
    required_hops: Iterable[object], collected_hops: Iterable[object]
) -> bool | None:
...
def _report(
    name: str, values: Mapping[str, float | None], categories: Mapping[str, str]
) -> MetricReport:
...
def evaluate_evidence(
    ...
    names = (
        "evidence_set_recall",
        "all_required_evidence@10",
        "cross_reference_resolution_recall",
        "multi_hop_evidence_completeness",
    )
    values: dict[str, dict[str, float | None]] = {name: {} for name in names}
    categories: dict[str, str] = {}
    for index, record in enumerate(records):
        key = str(record.get("id", index))
        required = cast(Iterable[object], record.get("required_evidence", []))
        categories[key] = str(record.get("category", "uncategorized"))
        covered = cast(
            Iterable[object],
            record.get("covered_evidence", record.get("retrieved_evidence", [])),
        )
        values["evidence_set_recall"][key] = evidence_set_recall(required, covered)
        top_ten = cast(Sequence[object], record.get("retrieved_evidence", []))[:10]
        all_required = all_required_evidence_at_10(required, top_ten)
        values["all_required_evidence@10"][key] = (
            float(all_required) if all_required is not None else None
        )
        expected_targets = cast(Iterable[str], record.get("expected_relation_targets", []))
        resolved_targets = cast(Iterable[str], record.get("resolved_relation_targets", []))
        values["cross_reference_resolution_recall"][key] = cross_reference_resolution_recall(
            expected_targets, resolved_targets
        )
        required_hops = cast(Iterable[object], record.get("required_hops", required))
        # Multi-hop is only meaningful for genuinely multi-evidence/multi-document queries.
        is_multi_hop = len(_as_set(required_hops)) > 1 or bool(record.get("multi_document"))
        if is_multi_hop:
            collected_hops = cast(Iterable[object], record.get("collected_hops", covered))
            complete = multi_hop_evidence_completeness(required_hops, collected_hops)
            values["multi_hop_evidence_completeness"][key] = (
                float(complete) if complete is not None else None
            )
        else:
            values["multi_hop_evidence_completeness"][key] = None
