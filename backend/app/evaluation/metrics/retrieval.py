...
def _aggregate(
    name: str, values: Mapping[str, float | None], categories: Mapping[str, str] | None = None
) -> MetricReport:
...
def evaluate_retrieval(
    ...
    values: dict[str, dict[str, float | None]] = {
        name: {} for name in ("recall@5", "recall@10", "recall@20", "mrr@10", "ndcg@10")
    }
    ...
    for index, record in enumerate(records):
        ...
        retrieved = [
            str(value)
            for value in cast(Sequence[object], record.get("retrieved", record.get("results", [])))
        ]
        relevant = cast(
            Iterable[object], record.get("relevant", record.get("expected_provision_ids", []))
        )
