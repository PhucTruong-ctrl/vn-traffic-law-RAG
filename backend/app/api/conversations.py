...
def _conversation(db: Session, ident: uuid.UUID, owner: str) -> Conversation:
    row = db.query(Conversation).filter(
        Conversation.id == ident, Conversation.owner_key == owner, Conversation.deleted_at.is_(None)
    ).first()
    if row is None:
...
def _summary(row: Conversation) -> dict[str, object]:
    return {k: getattr(row, k) for k in ("id", "title", "title_manual", "created_at", "updated_at", "last_activity_at")}


def _message(row: Message) -> dict[str, object]:
    return {"id": row.id, "role": row.role, "status": row.status, "content": row.content, "created_at": row.created_at, "query_trace_id": row.query_trace_id}
...
def list_conversations(
    ...
    query = db.query(Conversation).filter(Conversation.owner_key == key, Conversation.deleted_at.is_(None))
    if search and search.strip():
        needle = f"%{search.strip().casefold()}%"
        query = query.outerjoin(Message).filter(or_(Conversation.title.ilike(needle), (Message.role == "user") & Message.content.ilike(needle))).distinct()
    rows = query.order_by(Conversation.last_activity_at.desc(), Conversation.id.desc()).limit(limit + 1).all()
...
def get_conversation(
    ...
    return {**_summary(row), "messages": [_message(message) for message in row.messages if message.status != "FAILED"]}
