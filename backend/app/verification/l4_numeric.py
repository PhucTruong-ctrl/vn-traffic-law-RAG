...
class L4NumericVerifier:
    def verify(self, draft: Any, context: Sequence[Any] = (), query_context: Any = None, **_: Any) -> LayerResult:
        issues: list[VerificationIssue] = []; checked: list[str] = []
        ...
        for i, claim in enumerate(_v(draft, "claims", ()) or ()):
            ...
            for n in nums:
                if normalize_number(n) not in available:
                    issues.append(VerificationIssue("L4_NUMERIC_MISMATCH", f"number not grounded: {n}", claim_index=i))
            if nums and all(normalize_number(n) in available for n in nums): checked.append(str(i))
