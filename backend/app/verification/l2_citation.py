...
def _metadata_matches(citation: Any, record: Any) -> bool:
    return all(_value(citation, name) is None or _value(citation, name) == _value(record, name) for name in ("document_id", "document_number", "article", "clause", "point"))
...
class L2CitationVerifier:
    ...
    def verify(self, draft: Any, context: Sequence[Any] = (), query_context: Any = None, *, provisions: Any = None, expanded: Sequence[Any] = ()) -> LayerResult:
        ...
        for claim_index, claim in enumerate(_value(draft, "claims", ()) or ()):
            for citation in _value(claim, "citations", ()) or ():
                ...
                if not pid:
                    issues.append(VerificationIssue("L2_INVALID_CITATION", "citation has no provision_id", claim_index=claim_index)); continue
                record = known.get(pid)
                if record is None:
                    issues.append(VerificationIssue("L2_UNKNOWN_PROVISION", f"unknown provision_id: {pid}", claim_index=claim_index, provision_id=pid)); continue
                if pid not in whitelist and _value(record, "added_by") not in {"expanded", "context_expansion", "legal_context"}:
                    issues.append(VerificationIssue("L2_INVALID_CITATION", f"provision_id not in context whitelist: {pid}", claim_index=claim_index, provision_id=pid)); continue
                if _value(record, "review_status", "ACCEPTED") != "ACCEPTED":
                    issues.append(VerificationIssue("L2_INVALID_CITATION", f"provision is not accepted: {pid}", claim_index=claim_index, provision_id=pid)); continue
                if not _metadata_matches(citation, record):
                    issues.append(VerificationIssue("L2_INVALID_CITATION", f"citation metadata mismatch: {pid}", claim_index=claim_index, provision_id=pid)); continue
                checked.append(pid)
        return LayerResult(not issues, issues, checked)

def verify(draft: Any, context: Sequence[Any] = (), query_context: Any = None, **kwargs: Any) -> LayerResult:
