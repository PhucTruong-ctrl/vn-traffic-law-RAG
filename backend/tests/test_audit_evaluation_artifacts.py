...
def test_audit_preserves_incomplete_source_statuses(tmp_path: Path) -> None:
    ...
    report.write_text(json.dumps({"status": "PARTIAL", "sources": ["metrics.json"]}), encoding="utf-8")
...
def test_audit_blocks_missing_source_without_claiming_completion(tmp_path: Path) -> None:
    report = tmp_path / "report.json"
    report.write_text(json.dumps({"status": "COMPLETED", "sources": ["missing.json"]}), encoding="utf-8")
