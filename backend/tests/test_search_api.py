...
def _result(rank: int, number: str = "12/2024") -> SimpleNamespace:
    return SimpleNamespace(
        rank=rank, provision_id=f"p-{rank}", provision_version=1, document_id="d-1", document_version_id="dv-1",
        text="text", source_text="snippet", document_number=number, article="Điều 1", clause=None, point=None,
        effective_from=date(2024, 1, 1), effective_to=None, page_number=rank, retrieval_sources=["dense"], source_id="src", parent_context=None,
    )
...
def test_search_response_fields_filters_mode_and_pagination(
    ...
    response = TestClient(app).post("/api/v1/search", json={"query": "tax", "mode": mode, "document_type": "12", "page": 2, "page_size": 1})
    assert response.status_code == 200
    payload = response.json()
    assert payload["total"] == 2
    assert payload["page"] == 2 and payload["page_size"] == 1
    assert payload["results"][0]["provision_id"] == "p-3"
    assert {"rank", "provision_id", "interval", "hierarchy", "provenance", "snippet"} <= payload["results"][0].keys()
